...
export default function UploadCTView() {
  ...
  //   try {
  //     console.log("Inside patient email deatls");
  ...
  const generatePDF = async () => {
    ...
    console.log("Inside PDF function " + patientEmail);
    ...
    const nameText = `Name: ${response.data.name}`;
    const emailText = `Email: ${response.data.email}`;
    const gender = `Gender: ${response.data.gender}`;
    const phoneNo = `Phone Number: ${response.data.phoneNumber}`;
    const history = `History: ${response.data.medical_history}`;
    ...
    // Save the PDF
    doc.save(`${response.data.name} NephrolAI Report.pdf`);
  };
  ...
  const storeRD = useCallback(async () => {
    try {
    
      const response = await axios.post(
        `http://localhost:8080/admin/updateMonthlyReport`
      );
      ...
      {isLoading ? (
        ...
        <div>
          <div className="mt-3 flex space-x-20 ">
            <h2 className=" ml-[630px] text-3xl font-bold dark:text-white">
              Upload CT Scan
            </h2>
          </div>
          ...
          {errorsMessage && (
            <>
              <div
                className="relative ml-[600px] mt-5 w-60 rounded border border-red-400 bg-red-100 px-4 py-3 text-red-700"
                role="alert"
              >
          ...
          <div>
            <h2 className="ml-[600px] mt-5 text-2xl font-bold dark:text-white">
              Instruction For Uploading
            </h2>
            <ul className="mt-6 list-decimal">
              <li className="ml-[600px] text-base">
          ...
          <div>
            <label
              ...
              className="mb-2 ml-[600px] block text-sm font-medium text-gray-900 dark:text-white"
            >
              Enter Patient Email
            </label>
            <div className="relative mb-2">
              <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3.5">
                <svg
                  className="ml-[600px] h-4 w-4 text-gray-500 dark:text-gray-400"
                  aria-hidden="true"
              ...
              <input
                ...
                id="input-group-1"
                className="ml-[600px] block w-80 rounded-lg border border-gray-300 bg-gray-50 p-2.5 pl-10 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500  dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 dark:focus:border-blue-500 dark:focus:ring-blue-500"
          ...
          {isOpen && (
            <div className="fixed inset-0 z-50 flex animate-jump-in items-center justify-center overflow-auto ">
              ...
              <div className="modal-container z-50  rounded bg-white shadow-lg ">
                ...
                <button
                  className="mb-6 ml-24 mr-28 mt-5 flex rounded bg-slate-600 px-4  py-2 font-bold text-white hover:bg-slate-500"
                  onClick={generatePDF}
                >
                  Download Report
                  <svg
          ...
          <div class="bg-grey-lighter ml-[600px]  flex">
            <label class="hover: flex w-80 cursor-pointer flex-col items-center rounded-lg border border-slate-700 bg-white px-4 py-6 uppercase tracking-wide text-slate-700 shadow-lg">
              <svg
                class="h-8 w-8"
                fill="currentColor"
                xmlns="http://www.w3.org/2000/svg"
          ...
          <button
            ...
            className=" focus:ring-parimary ml-[700px] mt-5 rounded-lg bg-slate-700 px-5 py-2.5 text-center text-sm font-medium  text-white hover:bg-slate-800 focus:outline-none focus:ring-4 dark:bg-button dark:hover:bg-primary dark:focus:ring-button sm:w-auto lg:w-32"
          >
            Predict
          </button>
        </div>
